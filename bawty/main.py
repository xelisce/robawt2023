...
def task0_lt():
    ...
    if not gs_now and not red_now and not blue_now:
        ...
        #~ Image processing erode-dilate & crops
        curr = Task.EMPTY
        ...
        mask_uncropped_black = mask_black.copy() #to look for big line gap
        mask_supercrop_black = mask_black.copy() #to get width of line
        black_newlinegap_mask = mask_black.copy() #! ???

        mask_black[:horizon_crop_h, :] = 0
        mask_supercrop_black[:-gap_check_h-40, :] = 0  #! tune the crp here
        ...
        #~ Finding left and right index of first black line black (based off supercropped black)
        ...
        #~ Finding the start and the end of 2nd line segment
        ...
        print("2nd black: ", second_line_bottom, "2nd white:", second_line_top) #& debug height of second line
        ...
        #~ Finding left and right index of 2nd black line black
        if black_second_line_sum > 0: #^ getting (offsetted) width of second line segment 
            black_second_row = np.amax(black_second_line_mask[second_line_bottom-17:second_line_bottom, :], axis=0) #^ check bottom 17 pixels of 2nd line to find width
            ...
            black_second_line_mask_width = black_second_line_mask_end_x - black_second_line_mask_start_x

            print("second x width:", black_second_line_mask_width) #& debug width of line
            print("black second pixels sum:", black_second_line_sum)
            # cv2.imshow("second line", black_second_line_mask)

        else:
            black_second_line_mask_width = 0

        #^ supposedly accounts for the stupid gaps between tiles (hopefully works now)
        print("minigap", first_line_top - second_line_bottom)
        if first_line_top - second_line_bottom < 22 and first_line_top > 140 and second_line_bottom > 300: #? technically first_line_top > 140 is redundant but wtv
            first_line_top = second_line_top
            ...
            if first_line_top < 140:
                ...
                if len(cnts):
                    ...
                    # print("after contours, white start y:", first_line_top)
                    # cv2.imshow("contours", contour_mask)

                    mask_black = cv2.bitwise_and(mask_black, contour_mask)

        #~ If line gap (line ending and line width small: is this to check for sharp turns??) 
        if ((first_line_bottom < 460 or first_line_top > 300) and black_line_width < 300): #or (second_line_bottom > 150 and second_line_top < 150):
                if second_line_bottom <= 120 and not 80 <= black_second_line_mask_width <= 600: #or first_line_bottom < 200:
                    curr = Task.LINEGAP
                    print(">" * 25 + 'LINE GAP' + '<' * 25)
        ...
        if (first_line_bottom > 200 and first_line_top < 200 and black_line_width > 100) or (second_line_bottom > 120 and second_line_top < 120 and black_second_line_sum > 3500 and 80 < black_second_line_mask_width < 600): #!!! white2ndstart = 120, 80<black_second_line_mask_width<600
        ...
        # if black_line_height > 250 and 100 < black_line_width < 460 and black_start_x > 450 and first_line_top < 450:
        #     seesaw = 1
        # else:
        #     seesaw = 0
...
#     return (std[0][0] > min_std_dev and np.sum(st_mask_c) > min_mask_sum)import cv2
from MultiThread import WebcamStream
import numpy as np
import serial
import enum
...
def task0_lt():
    ...
    if not gs_now and not red_now and not blue_now:
        ...
        if black_second_line_sum > 0: #^ getting (offsetted) width of second line segment 
            black_second_row = np.amax(black_second_line_mask[second_line_bottom-35:second_line_bottom, :], axis=0)
            black_second_indices = np.where(black_second_row == 255)
            black_second_line_mask_start_x = black_second_indices[0][0] if len(black_second_indices[0]) else 0
            black_second_line_mask_end_x = black_second_indices[0][-1] if len(black_second_indices[0]) else 0
            black_second_line_mask_width = black_second_line_mask_end_x - black_second_line_mask_start_x
        ...
        #^ supposedly accounts for the stupid gaps between tiles (hopefully works now)
        print("minigap", first_line_top - second_line_bottom)
        if first_line_top - second_line_bottom < 11 and first_line_top > 140 and second_line_bottom > 300: #? technically first_line_top > 140 is redundant but wtv
            first_line_top = second_line_top
            black_line_height = first_line_bottom - first_line_top #^ adjusts the new height so bot stops wiggling
            print("/" * 20 + "minigap detected" + "\\" * 20)
            print("new first_line_top", first_line_top)
        ...
        if ((first_line_bottom < 230 or first_line_top > 150) and black_line_width < 150): #or (second_line_bottom > 150 and second_line_top < 150):
                if second_line_bottom <= 60 and not 40 <= black_second_line_mask_width <= 300: #or first_line_bottom < 200:
                    curr = Task.LINEGAP
                ...
                x_black_com = x_linegap_com
                powered_y = 1
                 #^ FOR LINEGAP DEBUGGING
                print ("LINEGAP DEBUGGING: first_line_bottom:", first_line_bottom)
        ...
        #^ First condition:
        #? Force bot to move forward when encoder vals remain relatively unchanged for ~5 sec?
        if (first_line_bottom > 100 and first_line_top < 100 and black_line_width > 50) or (second_line_bottom > 60 and second_line_top < 60 and black_second_line_sum > 1750 and 40 < black_second_line_mask_width < 300): #!!! white2ndstart = 120, 80<black_second_line_mask_width<600
            #!! whitestarty < 165, white_2nd_start < 110, 70<black_second_width<600
            curr = Task.EMPTY
            end_line_gap = 1
            x_black_com = x_com
        ...
        #^ used with alternate linegap
        if black_newlinegap_sum > 5000 and 40<new_mask_width<300 : #sum was previously 500
            new_endlinegap = 1
            print(";" * 43, "new linegap", ";"*43)
        else:
            new_endlinegap = 0
