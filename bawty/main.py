...
crop_h_bw = 93
crop_h_bw_gap = 110
gap_check_h = 75 # was 150, 170, 180, 155
horizon_crop_h = 20
...
def task0_lt():
    ...
    #~ Test for sufficient green in frame
        ...
        if gs_bot > 235:
            ...
            #~ Find y positions of green (FOR DEBUGGING)            
            green_row = np.amax(mask_gs, axis=1)
            g_indices_v =np.where(green_row==255) #v for vertical
            gs_top = g_indices_v[0][0]
            gs_bot = g_indices_v[0][-1]
            gs_height = gs_bot - gs_top
            print ("height of green sqr", gs_height)
    ...
    #* LINETRACK
    #^ CONTENT PAGE
    #^ first combine the lines
    # if minigap (which is tile gap): combine the lines
    # elif line long enough: contours
    #^ then analyse the line
    # if line gap: use a different x component and power (and if it is long line gap, do a sweep)
    # else is normal line track: crop and analyse line as normal
    #^ then check for the end of a line gap
    ...
    if not gs_now and not red_now and not blue_now:
        ...
        mask_supercrop_black[:-gap_check_h-20, :] = 0 
        mask_supercrop_black[-20:, :] = 0 
        ...
        #* VALUES OF LINES (LEFT RIGHT TOP BOTTOM)

        #~ Finding the top and bottom of closest line in UNCROPPED black
        ...
        black_row[first_line_bottom:] = 255
        white_indices = np.where(black_row == 0)
        first_line_top = white_indices[0][-1] if len(white_indices[0]) else 0

        black_line_height = first_line_bottom-first_line_top
        print("black start: ", first_line_bottom, "white: start", first_line_top)
        print("black line height: ", black_line_height)

        #~ Finding left and right of closest line (based off supercropped black)
        ...
        print("x width:", black_line_width) #& debug width of first line

        #~ Finding top and bottom of 2nd line segment
        black_row[first_line_top:] = 0
        black_indices_v2 = np.where(black_row == 255)
        second_line_bottom = black_indices_v2[0][-1] if len(black_indices_v2[0]) else 0
        black_row[second_line_bottom:] = 255
        white_indices_v2 = np.where(black_row == 0)
        second_line_top = white_indices_v2[0][-1] if len(white_indices_v2[0]) else 0
        print("2nd bottom: ", second_line_bottom, "2nd top:", second_line_top) #& debug height of second line


        #* UPDATES THE LINES 

        #~ Account for tile gap white by extending the line
        print("minigap", first_line_top - second_line_bottom)
        if first_line_top - second_line_bottom < 6 and second_line_bottom > 150:
            #^ explaning conditions
            #^ first_line_top - second_line_bottom < 6: ensures the gap between the lines are small enough to be a tile gap
            #^ second_line_bottom > 150: the second line is close enough to you
            first_line_top = second_line_top # combine the two lines so that we are tracking the entire thing
            black_line_height = first_line_bottom - first_line_top # adjusts the new height so bot stops wiggling
            print("/" * 20 + "minigap detected" + "\\" * 20)
            print("new first_line_top", first_line_top)
            print("new black line height: ", black_line_height)

            #~ Update second line top and bottom (if there are more minigaps due to many silver strips on edges of tiles)
            black_row[first_line_top:] = 0
            black_indices_v2 = np.where(black_row == 255)
            second_line_bottom = black_indices_v2[0][-1] if len(black_indices_v2[0]) else 0
            black_row[second_line_bottom:] = 255
            white_indices_v2 = np.where(black_row == 0)
            second_line_top = white_indices_v2[0][-1] if len(white_indices_v2[0]) else 0
            print("new 2nd bottom: ", second_line_bottom, "2nd top:", second_line_top) #& debug height of second line

            #~ Continue same process
            while first_line_top - second_line_bottom < 6 and second_line_bottom > 150:
                #^ explaning conditions
                #^ first_line_top - second_line_bottom < 6: ensures the gap between the lines are small enough to be a tile gap
                #^ second_line_bottom > 150: the second line is close enough to you
                first_line_top = second_line_top # combine the two lines so that we are tracking the entire thing
        ...
        #~ Contours to account for lines which overlap each other due to perspective of bottom camera (weird national tile)
        elif first_line_top < 35: #^ this only happens when the line is sufficiently long (otherwise it is highly unlikely that there is an overlap, or it might be likely that it is a tile gap instead)
            # cv2.imshow("before contours", mask_black)
            contours, _ = cv2.findContours(mask_black, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            #^ to reduce the time needed to do countors, cv2.RETR_EXTERNAL only searches for the most external contours (so it ignores any contours within the external contour)
            #^ and cv2.APPROX_SIMPLE only finds the corners
            ...
            if len(cnts):
                closest_contour = max(cnts, key=lambda x: x["y"]+x["h"]) #^ x and y is the lower left corner of the contour
                # first_line_top = int(closest_contour["y"] - closest_contour["h"])
                ...
                # print("after contours, first_line_top:", first_line_top)
                # cv2.imshow("contours", contour_mask)

                mask_black = cv2.bitwise_and(mask_black, contour_mask) #^ overwrite mask black to only contain the contour of line

        #* CONTINUATION OF FINDING CHARACTERISTICS OF LINES (TOP BOTTOM LEFT RIGHT)
        #~ Mask of 2nd line segment 
        black_second_line_mask = mask_black.copy()
        black_second_line_mask[:second_line_top, :] = 0
        black_second_line_mask[second_line_bottom:, :] = 0
        black_second_line_sum = np.sum(black_second_line_mask) /255

        #~ Finding the left and right of 2nd line segment
        if black_second_line_sum > 0:
            black_second_row = np.amax(black_second_line_mask[second_line_bottom-17:second_line_bottom, :], axis=0) #^ check bottom 17 pixels of 2nd line to check for width
            black_second_indices = np.where(black_second_row == 255)
            black_second_line_mask_start_x = black_second_indices[0][0] if len(black_second_indices[0]) else 0
            black_second_line_mask_end_x = black_second_indices[0][-1] if len(black_second_indices[0]) else 0
            black_second_line_width = black_second_line_mask_end_x - black_second_line_mask_start_x 

            print("second x width:", black_second_line_width) #& debug width of 2nd line
            print("black second pixels sum:", black_second_line_sum)
            # cv2.imshow("second line", black_second_line_mask)

        else:
            black_second_line_width = 0

        #* LINE GAP

        #~ If real line gap (line ending/ended and line width small)
        if ((first_line_bottom < 115 or first_line_top > 75) and black_line_width < 75):
            #^ first_line_bottom < 115: the line we were once on is already gone and the next line is far away
            #^ OR first_line_top > 75: the line before the gap is still visible, but ends soon
            #^ The previous two conditions, AND black_line_width < 75: the black line is thin (so that it doesn't trigger on 90)
            if second_line_bottom <= 30 and not 20 <= black_second_line_width <= 150: #or first_line_bottom < 200:
                #^ second_line_bottom <= 30: if the second line starts far away
                #^ not black_second_line_width <= 150: second line is not seeing outside the mat
                #^ not 20 <= black_second_line_width: second line is not the tile gap (vertical tile gap)
                curr = Task.LINEGAP #^ sweep for line
                print(">" * 15 + 'LINE GAP' + '<' * 15)
            mask_linegap_black = mask_black_org.copy() #^ open up the mask to look for faraway lines
            ...
            x_black_com = x_linegap_com #^ weigh the further lines more
            powered_y = 1
        ...
        #* NORMAL LINETRACK
            ...
            mask_black[:first_line_top, :] = 0 #^ only care about the line right in front, crop everything else out
            ...
            #~ Plain line track
            powered_y = (height_lt-20)/black_line_height if black_line_height != 0 else 1 #^ power > 1 
            powered_y = powered_y ** 0.3 #^ power still > 1, but we press down on the power so it's less aggressive
        ...
        #^ First condition:
        if (first_line_bottom > 50 and first_line_top < 50 and black_line_width > 25) or (second_line_bottom > 30 and second_line_top < 30 and black_second_line_sum > 875 and 40 < black_second_line_width < 300):
            #^ line starts near and ends far and is thick (so it's not the tile gap)
            #^ OR second line starts very close and ends very far and has alot of black but isn't too thick (to not be the grey area outside mat)
            curr = Task.EMPTY #^ overrides the line gap task basck to normal linetrack
            end_line_gap = 1
            x_black_com = x_com
