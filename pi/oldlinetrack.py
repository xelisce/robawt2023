...
while True:
    ...
    if not gs_now and not red_now and not blue_now:
        ...
        #~ Line gap
        if np.sum(mask_gap):
            x_gap_black = cv2.bitwise_and(x_gap_com, x_gap_com, mask = mask_black)
            gap_kernel = np.ones((5, 5), np.uint8)
            mask_gap = cv2.erode(mask_gap, gap_kernel) 
            mask_gap = cv2.dilate(mask_gap, gap_kernel)
            black_cols = np.amax(mask_gap, axis=0)
            black_indices_h = np.where(black_cols==255)

            black_rows = np.amax(mask_gap, axis=1)
            black_indices_v = np.where(black_rows==255)
            print(black_indices_v)

            if len(black_indices_h[0]):
                if black_indices_v[0][0] > 300: #there's still line left right in front but it ends
                    gap_left = black_indices_h[0][0]
                    gap_right = black_indices_h[0][-1]
                    print("Max black y:", np.max(y_black))
                    print("Left black:", gap_left)
                    print("Right black:", gap_right)
                    gap_blk_width = gap_right - gap_left

                    if gap_blk_width < 500 and np.max(y_black) > 0.97: #make sure theres line ahead
                        gap_now = True
                    else:
                        gap_now = False
                        
                    print("Black width", gap_right-gap_left)
                else: #doesnt end
                    gap_now = False
            else: #no line in front
                gap_now = True
        else: #no line completely in gap mask
            gap_now = False

        #~ Line track values
        y_resultant = np.mean(y_black)
        x_resultant = np.mean(x_gap_black) if gap_now else np.mean(x_black)
        print("Gap now:", gap_now)
