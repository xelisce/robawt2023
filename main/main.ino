...
currType afterTurnEvacState; 
currType afterPickupState = WALLTRACK;
...
bool evacLeftOut, evacRightOut;
unsigned long long lastSawEvacRightOutMillis, lastSawEvacLeftOutMillis;

int depositColor = 0;

//~ Evac Deposit
double startHeadingToDepositDist;
int pickupState, 
    evacExitState = 0,
...
int exitEvacTurnState;
double startEvacExitTurnDist, currEvacExitTurnDist, evacExitToTurnDist;
bool isFrontEvacExit = false;
unsigned long long lastTriggerRightSeeCloseWall;
    ...
    if (switch_on()) 
    {
        ...
        if (curr != STOP && curr != MOVE_DIST && curr != TURN_ANGLE && curr != TURN_TIME) {
            switch (task)
            {
                case 0: //EMPTY LINETRACK
                    ...
                    if (curr == CENTERING_FOR_DEPOSIT || curr == HEAD_TO_DEPOSIT || curr == DEPOSIT || curr == POST_DEPOSIT) {break;}
                    if (curr == REVERSE_FROM_ALIVE || curr == AFTER_DEAD) {break;}
                    if (curr == EXIT_EVAC_TURN) {
                        foundLine = true;
                        in_evac = false;
                    }
                ...
                case 4:
                    if (curr == EMPTY_LINETRACK && !front_see_wall()) {
                        curr = RED;
                    }
                    break;
                ...
                case 22: //^ deposit alive
                    //~ if curr isnt any of the following
                    if (!(curr == WALLTRACK || curr == WALLTRACK_OOR || curr == WALLTRACK_HITWALL || curr == WALLTRACK_WALLGAP || curr == CENTERING_FOR_DEPOSIT || curr == HEAD_TO_DEPOSIT)) { break; }
                    ...
                    if(depositType != 2) { depositType = 1; }
                    break;
                ...
                case 24: //^ empty evac
                    // if (!(curr == WALLTRACK_OOR || curr == WALLTRACK_HITWALL || curr == WALLTRACK_WALLGAP || curr == CENTERING_FOR_DEPOSIT || curr == HEAD_TO_DEPOSIT)) { break; }
                    // curr = CENTERING_FOR_DEPOSIT;
                    if (curr == POST_DEPOSIT) { curr = CENTERING_FOR_DEPOSIT; } 
                    break;
        ...
        switch (curr)
        {
            ...
            case LEFT_GREEN:
                ...
                turnByTime(-0.8, 1000, 110, EMPTY_LINETRACK);
                break;
            ...
            case LINEGAP:
                ...
                switch (linegapSweepState)
                {
                    ...
                    case 12:
                        ...
                        if ((lt2evacSawLeft && lt2evacSawRight) && !aboutToEnterEvac){ // If extrusions are detected
                            aboutToEnterEvac = true;
                            lastSentEvacEntryToPiMillis = millis();
                            Serial.print("lastsaw front left millis,"); Serial.println(lastSawFrontLeftMillis);
                            Serial.print("lastsaw front right millis,"); Serial.println(lastSawFrontRightMillis);
            ...
            case OBSTACLE: //^ going around obstacle
                ...
                switch (obstState)
                {
                    ...
                    case 4:
                        ledOn = true;
                        Robawt.setSteer(rpm, 0);
                        if (fabs(pickMotorDist(turn_dir) - obstCurrDist > 30)) {
                ...
                if (see_line && (millis() - obst_time_start) > 1000){ //previously 2 seconds;'
                    // moveDist(1, 30, 100, AFTER_OBSTACLE_TURN);
                    // curr = AFTER_OBSTACLE_TURN;
                    obstCurrDist = pickMotorDist(turn_dir); //tmp variable
                    obst_time_start = millis();
                    obstState = 4; 
                }
            ...
            case EVAC_INIT:
                ...
                switch (enterEvacState) {
                    ...
                    case 1: // move forward
                        Robawt.setSteer(150, -0.1);
                        if (fabs(pickMotorDist(-1) - evac_startDist) > 300) { //! arbitrary number
                            evac_startDist = pickMotorDist(-1);
                            enterEvacState++;
                        }
                        break;
            ...
            case EVAC_PICKUP:
                if (afterPickupState != CENTERING_FOR_DEPOSIT) { send_pi(Pi::BALLTRACK); }
                switch (pickupState)
                {
                    case 0:
            ...
            case WALLTRACK: //^ no ball walltrack
                ...
                if (right_see_reallyclosewall()){
                    evacRightOut = true;
                    lastSawEvacRightOutMillis = millis();
                }
                if (left_see_reallyclosewall()){
                    evacLeftOut = true;
                    lastSawEvacLeftOutMillis = millis();
                }
                if(front_see_wall()) {
                    evacLeftOut = false;
                    evacRightOut = false;
                }
                if (evacLeftOut && evacRightOut) { // If extrusions are detected
                ...
                // if (depositColor != 0) {
                //     turnAngle(-0.5, 270, 100, DEPOSIT_CHECKPOINT);
                // }
                break;

            // case DEPOSIT_CHECKPOINT:
            //     moveDist(1, 25, 100, DEPOSIT);
            //     depositNum = 1;
            //     afterDepositState = WALLTRACK;
            ...
            case WALLTRACK_OOR:
                switch (OORTurnState)
                {
                    case 0:
                        Robawt.setSteer(-120, 0);
                        if (fabs(MotorL.getDist() - startReverseOORDist) > 60) {  //x3
                            OORTurnState ++; 
                    ...
                    case 2:
                        Robawt.setSteer(120, 0);
                        if (fabs(MotorL.getDist() - startStraightOORDist) > 30) { //x3
            ...
            case WALLTRACK_HITWALL:
                switch (wallTurnState)
                {
                    case 0:
                        Robawt.setSteer(-120, 0);
                        if (fabs(MotorL.getDist() - startReverseOORDist) > 20) { //x3 
                            wallTurnState ++; 
            ...
            case WALLTRACK_WALLGAP: // when gap is on left
                switch (wallGapTurnState)
                {
                    case 0:
                        ...
                        else if (moveWallGapDist > 200) {  //tripled
                            curr = afterTurnEvacState; 
                            wallGapTurnState = 0; }
                }
                break;
            ...
            case CENTERING_FOR_DEPOSIT: 
                ...
                // } else if (OOR_exit_present()) {
                //     startReverseOORDist = MotorL.getDist();
                //     curr = WALLTRACK_OOR;
                //     OORTurnState = 0; 
                //     afterTurnEvacState = CENTERING_FOR_DEPOSIT; }
                // else if (wall_exit_present()) {
                //     startReverseOORDist = MotorL.getDist();
                //     curr = WALLTRACK_HITWALL;
                //     wallTurnState = 0; 
                //     afterTurnEvacState = CENTERING_FOR_DEPOSIT; }
            ...
            case HEAD_TO_DEPOSIT:
                ...
                switch (evacHeadToDepositState){
                    case 0:
                        // Robawt.setSteer(evac_rpm, 0);
                        //^ changing to pwm cuz apparently pid causes our bot to drift?
                        digitalWrite(MotorL.getPin1(), LOW);
                        analogWrite(MotorL.getPin2(), 180);

                        digitalWrite(MotorR.getPin1(), LOW);
                        analogWrite(MotorR.getPin2(), 180);
                        Serial.println("i am supposed to be moving");
                        Serial.println(MotorL.getPin1());
                        Serial.println((int)((150/210)* 255));
                        ...
                        if ((l1x_readings[L1X::FRONT_BOTTOM] < 60 || l0x_readings[L0X::FRONT] < 25) && !ball_present()) { // grind against deposit for a while
                            moveDist(1, 20, 100, HEAD_TO_DEPOSIT);
            ...
            case DEPOSIT:
                ...
                switch (depositState)
                {
                    ...
                    case 1:
                        ...
                        if (millis() - depositStateTimer > 500 && depositNum > 3) { 
                            depositStateTimer = millis();
                            depositState = 0; 
                            curr = afterDepositState; 
                        } else if (millis() - depositStateTimer > 500) {
                            depositNum ++;
                            depositState = 0;
                        }
                        break;
            ...
            case REVERSE_FROM_ALIVE:
                ...
                if (fabs(pickMotorDist(1) - startReverseDistAfterDepL) > 200) {
                    curr = CENTERING_FOR_DEPOSIT;
                    task = 24;
                }
                break;
            ...
            case AFTER_DEAD:
                ...
                switch (depositToExitState) {
                    ...
                    case 2:
                        ...
                        if (fabs(pickMotorDist(1) - depositToExitDist) > 50) //x3
                        {
                            depositToExitState = 0;
                            curr = EXIT_EVAC;
                        }
            ...
            case EXIT_EVAC_TURN:
                ...
                switch (exitEvacTurnState) 
                {
                    ...
                    case 1: //turn until right lidar sees extrusion / turned too much
                        Robawt.setSteer(evac_exit_rpm, 0.5);
                        currEvacExitTurnDist = pickMotorDist(-1);
                        if (right_see_closewall() || isFrontEvacExit) {
                            exitEvacTurnState ++;
                            startEvacExitTurnDist = pickMotorDist(-1);
                        } else if (fabs(currEvacExitTurnDist - startEvacExitTurnDist) > 115) {
                            exitEvacTurnState ++;
                            startEvacExitTurnDist = pickMotorDist(-1);
                        }
                    ...
                    case 2: //move back by a certain amount until see line
                        ...
                        if (foundLine) {
                            curr = EMPTY_LINETRACK;
                        } else if (fabs(currEvacExitTurnDist - startEvacExitTurnDist) > 70) {
                            exitEvacTurnState ++;
                        }
                        break;
                    ...
                    case 4: //move forward 30cm
                        Robawt.setSteer(evac_exit_rpm, 0);
                        currEvacExitTurnDist = pickMotorDist(-1);
                        if (fabs(currEvacExitTurnDist - startEvacExitTurnDist) > 90) {
                            curr = EXIT_EVAC;
                            exitEvacTurnState = 0;
                        }
                        break;
...
{   
    while (Serial1.available()) 
    {
        ...
        if (serialData == 255 || serialData == 254 || serialData == 253 || serialData == 252 || serialData == 251 || serialData == 250 || serialData == 249 || serialData == 248) {
...
bool OOR_present() {
    return ((front_see_evac_out && right_see_evac_exit_wall()) //if front sees out, and right either sees wall or OOR
...
bool front_top_see_infinity() {
    return (l0x_readings[L0X::FRONT_TOP] > 1680);
}

bool front_see_evac_out(){
    return (l0x_readings[L0X::FRONT] > 320);
}
...
bool front_see_further_out(){ 
    return (l0x_readings[L0X::FRONT] > 700);
}

bool front_top_see_out(){
    return (l0x_readings[L0X::FRONT] > 700);
}
...
bool right_see_evac_exit_wall() {
    return (l0x_readings[L0X::RIGHT] < 300);
}
...
bool OOR_exit_present() {
    return ((front_top_see_infinity() && right_see_wall()) //if front sees out, and right either sees wall or OOR
    || (front_top_see_infinity() && (left_see_infinity() || (frontLeft_see_infinity() && left_see_out()))));
}