...
while True:
    ...
    while True:
        ...
        if furthestLineIndex > 75:  # last 1/5 of frame, line is ending
            ...
            if std > 0.3 and data != b'\xff':   # can loosen, and check if angle is close to 0
            #if std > 0.4:
                print('Silver Strip')
                print(1, data)
    ...
    #     # if np.sum(black_mask):
    #     #     black_contours, hierarchy = cv2.findContours(black_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # extract outermost contours
    ...
    #     #             print('ycontour:', cy_contour)

    #     #             radius = int(min(cy_contour * 1.4, 60))    # 1.75 old
    ...
    #     #             black_circled_mask = cv2.bitwise_and(black_mask, black_mask, mask = black_circle_mask)
    #     #             black_square_mask = black_square_mask - black_circle_mask
    #     #             black_squared_mask = cv2.bitwise_and(black_mask, black_mask, mask = black_square_mask)
    ...
    #     #             print('Percentage Background Black:', percentage_background_black)
    #     #             print('Percentage Ball:', percentage_ball)
    #     #             # percentage ball threshold: 10%
    #     #             if percentage_background_black < 0.05 and percentage_ball > 0.3:
    ...
    #     #             if cy_ball > 30:   # if ball is nearby, turn to center on it
    ...
    #     # eroded_black_mask = cv2.erode(filtered_black_mask, ball_kernel)
    #     # dilated_black_mask = cv2.dilate(eroded_black_mask, ball_kernel)
    #     # # cv2.imshow('Eroded', eroded_black_mask[90:, :])

    #     # subtracted_mask = filtered_black_mask - dilated_black_mask
    #     # # cv2.imshow('Subtracted Mask', subtracted_mask)
    ...
    #     # if percentage_white > 0.03:  # original: 0.01
